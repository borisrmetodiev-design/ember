...
async function executeResume(context, isSlash) {
    ...
    if (!queue.paused) {
        const e = buildMusicErrorEmbed("Playback is not paused.");
        return isSlash ? context.reply({ embeds: [e], flags: MessageFlags.Ephemeral }) : context.reply({ embeds: [e] });
    }

    try {
        MusicService.resume(guild.id);
        const embed = buildInfoEmbed("▶ Resumed", `Resumed **${queue.current.title}**.`);
...
module.exports = {
    ...
    async executeSlash(interaction) { await executeResume(interaction, true); },
    async executePrefix(message, args) { await executeResume(message, false); },
};
